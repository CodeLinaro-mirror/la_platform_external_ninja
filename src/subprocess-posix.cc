...
bool Subprocess::Start(SubprocessSet* set, const EdgeCommand& cmd, Edge* edge,
  ...
  if (!use_nsjail) {
    ...
    auto usrBinMount = nsjailConfig.add_mount();
    usrBinMount->set_src("/usr/bin");
    usrBinMount->set_dst("/usr/bin");
    usrBinMount->set_is_bind(true);
    usrBinMount->set_is_dir(true);
    auto usrLibMount = nsjailConfig.add_mount();
    usrLibMount->set_src("/usr/lib");
    usrLibMount->set_dst("/usr/lib");
    usrLibMount->set_is_bind(true);
    usrLibMount->set_is_dir(true);
    auto usrLib32Mount = nsjailConfig.add_mount();
    usrLib32Mount->set_src("/usr/lib32");
    usrLib32Mount->set_dst("/usr/lib32");
    usrLib32Mount->set_is_bind(true);
    usrLib32Mount->set_is_dir(true);
    auto usrLib64Mount = nsjailConfig.add_mount();
    usrLib64Mount->set_src("/usr/lib64");
    usrLib64Mount->set_dst("/usr/lib64");
    usrLib64Mount->set_is_bind(true);
    usrLib64Mount->set_is_dir(true);
